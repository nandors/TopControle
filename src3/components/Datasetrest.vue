...
//      this.$store.dispatch('atualizaToken')
      vm.atualizatoken().then(function(tokenf){
        axios.get('https://vspa-15b54.firebaseio.com/lojas.json?auth='+tokenf)
        .then(function(res){
...
//      mapRef.push({'lat': maplat, 'lng': maplng})
      ...
      if (this.$refs.form.validate()) {
        this.atualizatoken().then(function(tokenf){
          axios.post('https://vspa-15b54.firebaseio.com/lojas.json?auth='+tokenf, rg
          )
          .then(response => {
            vm.clear()
            vm.close()
...
//        tabelaRef.push(this.newRegistro)
    ...
    this.atualizatoken().then(function(tokenf){
      axios.put('https://vspa-15b54.firebaseio.com/lojas/'+ key + '.json?auth='+tokenf , vm.newRegistro
      )
      .then(response => {
        vm.clear()
        const btnUpdate = document.getElementById('btnUp')
    ...
    removeRegistro: function (tabela) {
      ...
      this.atualizatoken().then(function(tokenf){
        confirm('Confirma a exclusão?') &&
          axios.delete('https://vspa-15b54.firebaseio.com/lojas/'+ tabela['key'] + '.json?auth='+tokenf)
          .then(response => {
